...
One agent is easy: you watch the terminal. Many agents are not. On this Mac, right now: **19 sessions running, 11 of them waiting on a human**. Over 30 days, the sessions a human was actually sitting in stopped **441 times**. Half were picked up within ten minutes — and **111 of them sat for more than half an hour, 3.2 waking hours each on average** (351 hours in total, nights excluded). Silently, because a stopped agent prints nothing, and macOS notifications turn out to have been denied on this machine the whole time. Unattended `claude -p` runs stopped another 880 times and nobody ever came back to those. (Measured 2026-09-23. These figures cannot be re-checked later: Claude Code deletes transcripts after 30 days, so the source data for any past measurement is already gone — see [docs/why.md](docs/why.md). What is reproducible is the counting method, in `board/recovery.py`, not the numbers.)
