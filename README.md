...
One agent is easy: you watch the terminal. Many agents are not. On this Mac, right now: **19 sessions running, 11 of them waiting on a human**. Over 30 days, the sessions a human was actually sitting in stopped **428 times**. Half got a next message within ten minutes — and **110 of them sat for more than half an hour**: **194 hours of wall-clock time** with at least one session stalled, nights excluded. (Adding each stall separately gives 387 hours, but half of that is two sessions stalled at once, so it is not 387 hours of anyone's life.) Silently, because a stopped agent prints nothing, and macOS notifications turn out to have been denied on this machine the whole time. Unattended `claude -p` runs stopped another 870 times and nobody ever came back to those. (Measured 2026-09-24. This counts time to *respond*, not time to *notice* — the log only holds your next message. These figures cannot be re-checked later: Claude Code deletes transcripts after 30 days, so the source data for any past measurement is already gone — see [docs/why.md](docs/why.md). What is reproducible is the counting method, in `board/recovery.py`, not the numbers.)
