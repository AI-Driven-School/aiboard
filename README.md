...
One agent is easy: you watch the terminal. Many agents are not. On this Mac, right now: **19 sessions running, 11 of them waiting on a human**. Over 30 days, the sessions a human was actually sitting in stopped **476 times**. Half were picked up within ten minutes — and **124 of them sat for more than half an hour, 3.2 waking hours each on average** (400 hours in total, nights excluded). Silently, because a stopped agent prints nothing, and macOS notifications turn out to have been denied on this machine the whole time. Unattended `claude -p` runs stopped another 881 times and nobody ever came back to those.
